...
/* ══════════════════════════════════════════
   student-mat.php
   Prédiction via FastAPI (plus de shell_exec)
   ══════════════════════════════════════════ */

// ── URL de ton serveur FastAPI ─────────────────────────────────────────────
// En local         : http://localhost:8000/predict
// Sur Railway/Render: https://TON-APP.railway.app/predict
// ⚠ À MODIFIER selon l'endroit où FastAPI tourne
define('FASTAPI_URL', 'http://localhost:8000/predict');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'predict') {
    ...
    // ── 2. Construire le JSON à envoyer à FastAPI ──
    // On force les types entiers pour les champs numériques
    $payload = json_encode([
        'age'        => (int)$data['age'],
        'G1'         => (int)$data['G1'],
        'G2'         => (int)$data['G2'],
    ...
    // ── 3. Appel HTTP vers FastAPI via cURL ──
    // cURL est disponible sur tous les hébergements (y compris InfinityFree)
    // contrairement à shell_exec qui était bloqué
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_TIMEOUT        => 10,     // abandon après 10 secondes
    ...
    if ($curl_err) {
        // Erreur réseau : FastAPI injoignable (pas démarré, mauvaise URL...)
        echo json_encode(['error' => "FastAPI injoignable : $curl_err"]);
        exit;
    }

    if ($http_code !== 200) {
        echo json_encode(['error' => "FastAPI a répondu HTTP $http_code", 'raw' => $response]);
        exit;
    }

    // ── 5. Relayer la réponse JSON de FastAPI au navigateur ──
    $result = json_decode($response, true);
    if (!$result) {
        echo json_encode(['error' => 'Réponse FastAPI invalide', 'raw' => $response]);
